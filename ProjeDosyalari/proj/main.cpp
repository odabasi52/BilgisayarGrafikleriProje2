...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
  ...
  if (fpsCamera)
    cam.updateDirection(xoffset, 0.0f);
  else
    cam.updateDirection(xoffset, yoffset);
}
...
void Fkey_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
  if (key == GLFW_KEY_F && action == GLFW_PRESS) {
    fpsCamera = !fpsCamera;

    if (fpsCamera) {
      // Lock camera to ball start pos
      cam.yaw = -90.0f; // facing -Z
      cam.pitch = 0.0f; // lock pitch
    } else {
...
void processInput(GLFWwindow *window)
{
  if (fpsCamera) return; // disable movement in FPS mode
...
void load_all()
{
  ...
  cam.pos = glm::vec3(6.f, 1.7f, -1.f);
  cam.pos = glm::vec3(6.f, 1.7f, -1.f);
  cam.yaw = -90.0f;  // facing negative Z
  cam.pitch = 0.0f;
  cam.updateViewMatrix();
  ...
  if (debug)
  {
    ...
    cam.yaw = -90.0f;  // facing -Z
    cam.pitch = 0.0f;
    cam.updateViewMatrix();  // recalculate front/right/up
